- [Redis核心技术与实战](#redis核心技术与实战)
	- [数据结构](#数据结构)
		...
		- [查找复杂度](#查找复杂度)
		- [Redis IO 模型](#redis-io-模型)
	- [AOF和RDB](#aof和rdb)
		- [AOF 重写](#aof-重写)
		- [RDB](#rdb)
		- [RBD是不是可以每秒做一次快照？](#rbd是不是可以每秒做一次快照)
	- [高可用](#高可用)
		- [主从](#主从)
			- [主从库第一次同步的流程](#主从库第一次同步的流程)
		- [哨兵](#哨兵)
			- [哨兵机制的基本流程](#哨兵机制的基本流程)
			- [主观下线和客观下线](#主观下线和客观下线)
			- [如何选定新主库？](#如何选定新主库)
			- [哨兵在操作主从切换的过程中，客户端能否正常地进行请求操作？](#哨兵在操作主从切换的过程中客户端能否正常地进行请求操作)
			- [基于 pub/sub 机制的哨兵集群组成](#基于-pubsub-机制的哨兵集群组成)
			- [哨兵是如何知道从库的 IP 地址和端口的呢？](#哨兵是如何知道从库的-ip-地址和端口的呢)
			- [基于 pub/sub 机制的客户端事件通知](#基于-pubsub-机制的客户端事件通知)
			- [由哪个哨兵执行主从切换？](#由哪个哨兵执行主从切换)
		- [Redis集群](#redis集群)
			- [数据切片和实例的对应分布关系](#数据切片和实例的对应分布关系)
			- [客户端如何定位数据？](#客户端如何定位数据)
	- [问题](#问题)
		- [整数数组和压缩列表在查找时间复杂度方面并没有很大的优势，那为什么 Redis 还会把它们作为底层数据结构呢？](#整数数组和压缩列表在查找时间复杂度方面并没有很大的优势那为什么-redis-还会把它们作为底层数据结构呢)
		- [Redis 基本 IO 模型中还有哪些潜在的性能瓶颈？](#redis-基本-io-模型中还有哪些潜在的性能瓶颈)
		- [AOF 重写过程中有没有其他潜在的阻塞风险？](#aof-重写过程中有没有其他潜在的阻塞风险)
		- [AOF 重写为什么不共享使用 AOF 本身的日志？](#aof-重写为什么不共享使用-aof-本身的日志)
		- [使用一个 2 核 CPU、4GB 内存、500GB 磁盘的云主机运行 Redis，Redis 数据库的数据量大小差不多是 2GB。当时 Redis 主要以修改操作为主，写读比例差不多在 8:2 左右，也就是说，如果有 100 个请求，80 个请求执行的是修改操作。在这个场景下，用 RDB 做持久化有什么风险吗？](#使用一个-2-核-cpu4gb-内存500gb-磁盘的云主机运行-redisredis-数据库的数据量大小差不多是-2gb当时-redis-主要以修改操作为主写读比例差不多在-82-左右也就是说如果有-100-个请求80-个请求执行的是修改操作在这个场景下用-rdb-做持久化有什么风险吗)
		- [为什么主从库间的复制不使用 AOF？](#为什么主从库间的复制不使用-aof)
		- [在主从切换过程中，客户端能否正常地进行请求操作呢？](#在主从切换过程中客户端能否正常地进行请求操作呢)
		- [如果想要应用程序不感知服务的中断，还需要哨兵或客户端再做些什么吗？](#如果想要应用程序不感知服务的中断还需要哨兵或客户端再做些什么吗)
		- [哨兵实例是不是越多越好呢？如果同时调大 down-after-milliseconds 值，对减少误判是不是也有好处？](#哨兵实例是不是越多越好呢如果同时调大-down-after-milliseconds-值对减少误判是不是也有好处)
		- [为什么 Redis 不直接用一个表，把键值对和实例的对应关系记录下来？](#为什么-redis-不直接用一个表把键值对和实例的对应关系记录下来)
		- [Redis 什么时候做 rehash？](#redis-什么时候做-rehash)
		- [采用渐进式 hash 时，如果实例暂时没有收到新请求，是不是就不做 rehash 了？](#采用渐进式-hash-时如果实例暂时没有收到新请求是不是就不做-rehash-了)
		- [主线程、子进程和后台线程的联系与区别](#主线程子进程和后台线程的联系与区别)
		- [写时复制的底层实现机制](#写时复制的底层实现机制)
		- [replication buffer 和 repl_backlog_buffer 的区别](#replication-buffer-和-repl_backlog_buffer-的区别)
	- [实战](#实战)
		- [SDS为什么占空间](#sds为什么占空间)
		- [异步的子线程机制](#异步的子线程机制)
		- [绑核的风险和解决方案](#绑核的风险和解决方案)
			- [一个 Redis 实例对应绑一个物理核](#一个-redis-实例对应绑一个物理核)
			- [优化 Redis 源码](#优化-redis-源码)
		- [客户端输入和输出缓冲区](#客户端输入和输出缓冲区)
		- [问题二](#问题二)
			- [CPU问题](#cpu问题)
			- [Redis 变慢的情况](#redis-变慢的情况)
			- [缓存不一致问题](#缓存不一致问题)
			- [缓存雪崩、击穿、穿透](#缓存雪崩击穿穿透)
			- [Redis 可以在秒杀场景的哪些环节发挥作用？](#redis-可以在秒杀场景的哪些环节发挥作用)
			- [数据量倾斜的成因？](#数据量倾斜的成因)
				- [bigkey 导致倾斜](#bigkey-导致倾斜)
				- [Slot 分配不均衡导致倾斜](#slot-分配不均衡导致倾斜)
				- [Hash Tag 导致倾斜](#hash-tag-导致倾斜)
			- [数据访问倾斜的成因和应对方法？](#数据访问倾斜的成因和应对方法)
	- [Redis Cluster 瓶颈](#redis-cluster-瓶颈)
		- [实例间通信频率](#实例间通信频率)
		- [如何降低实例间的通信开销？](#如何降低实例间的通信开销)
	- [Redis 6.0新特性](#redis-60新特性)
		- [从单线程处理网络请求到多线程处理](#从单线程处理网络请求到多线程处理)
	- [Redis 和 Memcached、RocksDB 的对比](#redis-和-memcachedrocksdb-的对比)
