- [背景](#背景)
  ...
  - [etcd 基础](#etcd-基础)
  - [etcd 实践](#etcd-实践)
  - [etcd v2 功能](#etcd-v2-功能)
  - [etcd v2 存在的问题](#etcd-v2-存在的问题)
  - [k8s 为什么使用 etcd](#k8s-为什么使用-etcd)
  - [etcd v3 优化了什么？](#etcd-v3-优化了什么)
  - [etcd v3  基础架构](#etcd-v3--基础架构)
- [一致性读](#一致性读)
  - [串行读(Serializable)](#串行读serializable)
  - [线性读](#线性读)
- [MVCC](#mvcc)
  - [treeIndex](#treeindex)
  - [Revision概念](#revision概念)
  - [buffer](#buffer)
  - [boltdb](#boltdb)
- [etcd一个写请求是如何执行的？](#etcd一个写请求是如何执行的)
  - [Quota 模块](#quota-模块)
  - [KVServer 模块](#kvserver-模块)
  ...
  - [MVCC 模块](#mvcc-模块)
    ...
    - [boltdb](#boltdb-1)
- [鉴权：如何保护你的数据安全？](#鉴权如何保护你的数据安全)
- [Lease](#lease)
  - [key 如何关联 Lease](#key-如何关联-lease)
  - [如何优化 Lease 续期性能](#如何优化-lease-续期性能)
  - [如何高效淘汰过期 Lease](#如何高效淘汰过期-lease)
- [MVCC：如何实现多版本并发控制？](#mvcc如何实现多版本并发控制)
  - [treeIndex 原理](#treeindex-原理)
  - [MVCC 更新 key 原理](#mvcc-更新-key-原理)
  - [MVCC 查询 key 原理](#mvcc-查询-key-原理)
  - [MVCC 删除 key 原理](#mvcc-删除-key-原理)
- [Watche](#watche)
  - [滑动窗口 vs MVCC](#滑动窗口-vs-mvcc)
  - [可靠的事件推送机制](#可靠的事件推送机制)
  - [异常场景重试机制](#异常场景重试机制)
  - [历史事件推送机制](#历史事件推送机制)
  - [高效的事件匹配](#高效的事件匹配)
- [事务实现](#事务实现)
  - [事务特性初体验及 API](#事务特性初体验及-api)
- [boltdb：如何持久化存储你的key-value数据？](#boltdb如何持久化存储你的key-value数据)
  - [boltdb 磁盘布局](#boltdb-磁盘布局)
- [如何回收旧版本数据？](#如何回收旧版本数据)
  - [压缩原理](#压缩原理)
- [db大小：为什么etcd社区建议db大小不超过8G？](#db大小为什么etcd社区建议db大小不超过8g)
- [延时：为什么你的etcd请求会出现超时？](#延时为什么你的etcd请求会出现超时)
- [内存：为什么你的etcd内存占用那么高？](#内存为什么你的etcd内存占用那么高)
- [如何优化及扩展etcd性能？](#如何优化及扩展etcd性能)
- [ZooKeeper 架构及原理](#zookeeper-架构及原理)
- [Consul 架构及原理](#consul-架构及原理)
- [etcd vs ZooKeeper vs Consul](#etcd-vs-zookeeper-vs-consul)
  - [并发原语](#并发原语)
  - [健康检查、服务发现](#健康检查服务发现)
  - [数据模型比较](#数据模型比较)
  - [Watch 特性比较](#watch-特性比较)
  - [其他比较](#其他比较)
- [如何构建高可靠的etcd集群运维体系](#如何构建高可靠的etcd集群运维体系)
