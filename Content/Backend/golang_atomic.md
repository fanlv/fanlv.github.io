...
由于`CPU Cache`缓存数据最小的单位是一个Cache Line（64节），就会引入`Flase Sharing`问题。
...
Cache Line缓存测试
	...
	横向遍历耗时： 485995  //因为横向写数据的试试，会一直命中CPU缓存，所以比纵向更快一些
	纵向遍历耗时： 1705150
...
我们可以看到golang里面 p struct的源码里面最后一个属性是`pad cpu.CacheLinePad `
...
我们可以得到一个 `Set大小 = 8 * Cache Line = 512Byte` `Set个数 = 32*1024 /512 = 64` `Cache Line Count = 32*1024 / 64 = 512个`
...
* 直接映射（direct mapped cache），相当于每个set只有1个`cache line`（E=1）。那么相隔2^(s+b)个单元的2个内存单元，会被映射到同一个`cacheline`中。
