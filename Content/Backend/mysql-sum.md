- [1. Mysql 基础知识汇总](#1-mysql-%E5%9F%BA%E7%A1%80%E7%9F%A5%E8%AF%86%E6%B1%87%E6%80%BB)
  ...
  - [1.3. Mysql 物理文件组成详解](#13-mysql-%E7%89%A9%E7%90%86%E6%96%87%E4%BB%B6%E7%BB%84%E6%88%90%E8%AF%A6%E8%A7%A3)
    ...
    - [1.3.2. Binlog、Redolog、Undolog](#132-binlogredologundolog)
      ...
      - [有了Redolog，为啥还需要Binlog呢？](#%E6%9C%89%E4%BA%86redolog%E4%B8%BA%E5%95%A5%E8%BF%98%E9%9C%80%E8%A6%81binlog%E5%91%A2)
  ...
  - [1.6 数据库Sharding](#16-%E6%95%B0%E6%8D%AE%E5%BA%93sharding)
    ...
    - [1.6.2 常见Sharding方案](#162-%E5%B8%B8%E8%A7%81sharding%E6%96%B9%E6%A1%88)
      - [基于功能切分 （垂直切分）](#%E5%9F%BA%E4%BA%8E%E5%8A%9F%E8%83%BD%E5%88%87%E5%88%86-%E5%9E%82%E7%9B%B4%E5%88%87%E5%88%86)
      - [基于区间范围切分](#%E5%9F%BA%E4%BA%8E%E5%8C%BA%E9%97%B4%E8%8C%83%E5%9B%B4%E5%88%87%E5%88%86)
      - [基于hash切分](#%E5%9F%BA%E4%BA%8Ehash%E5%88%87%E5%88%86)
      - [基于路由表切分](#%E5%9F%BA%E4%BA%8E%E8%B7%AF%E7%94%B1%E8%A1%A8%E5%88%87%E5%88%86)
    - [1.6.3. Sharding优点](#163-sharding%E4%BC%98%E7%82%B9)
    - [1.6.4. 常见Sharding方案对比](#164-%E5%B8%B8%E8%A7%81sharding%E6%96%B9%E6%A1%88%E5%AF%B9%E6%AF%94)
    - [1.6.4. 参考](#164-%E5%8F%82%E8%80%83)
...
[关于MySQL自增主键的几点问题（上）](http://seanlook.com/2017/02/16/mysql-autoincrement/)
...
2. 数据迁移    
	在设计初期，由于没有足够的数据作为Sharding的参考，简单的方式就是把数据平均分到不同shard上，如每个shard均存放10000000用户的数据。系统上线后发现，如果用户id采用自增方式分配，即越早注册用户uid越小。通常情况下，系统早期用户都是比较活跃的用户。由于shard1上的用户更活跃，那么它的负载高于其他shard，当其性能出现瓶颈时就需要进行数据迁移。 
	...
	1. 多区间Sharding 即一个shard存储多个区间的数据。针对上述连锁迁移问题，先扩容出一个shard4，其负责的新用户范围为[30000001, 35000000]，并将[1, 5000000]范围内的用户数据迁移到shard4上。mongodb就是使用多区间Sharding的方案进行扩展。
	![](./images/1-3.png)
...
2. 二叉树扩展     
	二叉树扩展也能够很好地解决迁移混乱问题，redis在rehash渐进式迁移时就采用的这种方案。二叉树扩展要求shard的个数为2n个，只要满足此要求即可进行二叉树扩展。下面我们图解数据库如何进行二叉树扩展 ：   
	![](./images/1-8.png)
...
1. 基于功能切分是纯粹业务上的隔离，更多的是业务层面的考量。
2. 基于区间范围切分非常适合处理有区间查询需求的场景，但是各个shard上负载的均衡很难保证。
3. 基于hash切分能够较均匀地分配数据，但是扩展比较麻烦，需要保证hash算法单调性。
4. 基于路由表切分，最灵活、最通用、最容易扩展和迁移，但是需要额外维护路由数据。


### 1.6.4. 参考
[数据库Sharding技术及扩展方案](http://note.youdao.com/publicshare/?id=4217a59d87f7fd75b7d52d5a)
