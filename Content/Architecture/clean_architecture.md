- [零、概述](#零概述)
  - [0.1 设计与架构究竟是什么？](#01-设计与架构究竟是什么)
    - [目标](#目标)
  - [0.2 两个纬度的价值](#02-两个纬度的价值)
    - [行为价值](#行为价值)
    - [架构价值](#架构价值)
- [一、编程范式](#一编程范式)
  - [1.1 结构化编程（Structured Programming）](#11-结构化编程structured-programming)
  - [1.2 面向对象编程（Object Oriented Programming）](#12-面向对象编程object-oriented-programming)
  - [1.3 函数式编程（Functional Programming）](#13-函数式编程functional-programming)
  - [1.4 总结](#14-总结)
- [二、设计原则 - SOLID 原则](#二设计原则---solid-原则)
  - [2.1 单一职责（SRP）](#21-单一职责srp)
  - [2.2 开闭原则（OCP）](#22-开闭原则ocp)
  - [2.3 里式替换（LSP）](#23-里式替换lsp)
  - [2.4 接口隔离原则（ISP)](#24-接口隔离原则isp)
  - [2.5 依赖反转原则（DIP)](#25-依赖反转原则dip)
- [三、组件](#三组件)
  - [3.1 什么是组件](#31-什么是组件)
  - [3.2 组件的聚合](#32-组件的聚合)
    - [3.2.1 复用/发布等同原则（REP原则 - Release Reuse Equivalency Principle）](#321-复用发布等同原则rep原则---release-reuse-equivalency-principle)
    - [3.2.2 共同闭包原则 （CCP原则 - the Common Closure Principle）](#322-共同闭包原则-ccp原则---the-common-closure-principle)
      - [CCP与SRP原则的相似点:](#ccp与srp原则的相似点)
    - [3.2.3 共同复用原则（CRP原则 - Composite Reuse Principle）](#323-共同复用原则crp原则---composite-reuse-principle)
      - [与ISP原则的关系](#与isp原则的关系)
    - [3.2.4 三个规则的Treadeoff](#324-三个规则的treadeoff)
  - [3.3 组件耦合](#33-组件耦合)
    - [3.3.1 无依赖环原则（ADP - Acyclic Dependency Principle）](#331-无依赖环原则adp---acyclic-dependency-principle)
    - [3.3.2 自上而下的设计](#332-自上而下的设计)
    - [3.3.3 稳定依赖原则 （SDP - Stabilization Dependency Principle）](#333-稳定依赖原则-sdp---stabilization-dependency-principle)
    - [3.3.4 稳定抽象原则（SAP - Stable Abstractions Principle）](#334-稳定抽象原则sap---stable-abstractions-principle)
      - [3.3.5 高阶策略应该放在哪里？](#335-高阶策略应该放在哪里)
      - [3.3.6 稳定抽象原则简介](#336-稳定抽象原则简介)
- [四、软件架构](#四软件架构)
  - [4.1 什么是软件架构？软件架构师工作内容是什么？](#41-什么是软件架构软件架构师工作内容是什么)
    - [4.1.1 开发（development）](#411-开发development)
    - [4.1.2 部署（Deployment）](#412-部署deployment)
    - [4.1.3 维护（Maintenance）](#413-维护maintenance)
    - [4.1.3 保持可选项](#413-保持可选项)
  - [4.2 独立性](#42-独立性)
    - [4.2.1 用例](#421-用例)
    - [4.2.2 运行](#422-运行)
    - [4.2.3 开发](#423-开发)
    - [4.2.4 部署](#424-部署)
    - [4.2.5 保留可选项](#425-保留可选项)
    - [4.2.6 按层解耦](#426-按层解耦)
    - [4.2.7 用例的解耦](#427-用例的解耦)
    - [4.2.8 结构模式](#428-结构模式)
    - [4.2.9 开发独立性](#429-开发独立性)
    - [4.2.10 部署独立性](#4210-部署独立性)
    - [4.2.11 重复](#4211-重复)
    - [4.2.12 再谈解耦模式](#4212-再谈解耦模式)
  - [4.3 划分边界](#43-划分边界)
    - [4.3.1 插件式架构](#431-插件式架构)
    - [4.3.3 插件式架构好处](#433-插件式架构好处)
  - [4.4 边界剖析](#44-边界剖析)
    - [4.4.1 跨边界调用](#441-跨边界调用)
    - [4.4.2 本地进程](#442-本地进程)
    - [4.4.3 服务](#443-服务)
  - [4.5 策略和层次](#45-策略和层次)
    - [4.5.1 层次](#451-层次)
  - [4.6 业务逻辑](#46-业务逻辑)
    - [4.6.1 业务实体](#461-业务实体)
    - [4.6.2 用例](#462-用例)
  - [4.7 尖叫的软件架构](#47-尖叫的软件架构)
    - [4.7.1 架构设计的主题](#471-架构设计的主题)
    - [4.7.2 架构设计的核心目标](#472-架构设计的核心目标)
    - [4.7.3 框架是工具不是生活信条](#473-框架是工具不是生活信条)
    - [4.7.4 可测试的框架设计](#474-可测试的框架设计)
  - [4.8 整洁架构](#48-整洁架构)
    - [4.8.1  依赖关系规则](#481--依赖关系规则)
    - [4.8.2 一个场景应用场景](#482-一个场景应用场景)
  - [4.9 展示器和谦卑对象](#49-展示器和谦卑对象)
    - [4.9.1 谦卑对象模式](#491-谦卑对象模式)
    - [4.9.2  展示器与视图](#492--展示器与视图)
    - [4.9.3 测试与架构](#493-测试与架构)
    - [4.9.4 数据库网关](#494-数据库网关)
    - [4.9.5 数据映射器](#495-数据映射器)
    - [4.9.6 服务监听器](#496-服务监听器)
    - [4.9.7 不完全边界](#497-不完全边界)
    - [4.9.8 单向边界](#498-单向边界)
    - [4.9.9 门户模式](#499-门户模式)
  - [4.10 层次与边界](#410-层次与边界)
  - [4.11 Main组件](#411-main组件)
  - [4.12 宏观和微观](#412-宏观和微观)
  - [4.13 测试边界](#413-测试边界)
  - [4.14 整洁的嵌入式架构](#414-整洁的嵌入式架构)
- [五、实现细节](#五实现细节)
  - [5.1 数据库只是细节](#51-数据库只是细节)
    - [5.1.1 假设磁盘不存在会怎么样？](#511-假设磁盘不存在会怎么样)
    - [5.1.2 实现细节](#512-实现细节)
  - [5.2 web只是细节](#52-web只是细节)
  - [5.3 应用程序框架只是细节](#53-应用程序框架只是细节)
    - [5.3.1 单向婚姻](#531-单向婚姻)
    - [5.3.2 风险](#532-风险)
    - [5.3.3 解决方案](#533-解决方案)
    - [5.3.4 总结](#534-总结)
- [六、代码结构组织的几种方式](#六代码结构组织的几种方式)
  - [6.1 按层封装](#61-按层封装)
  - [6.2 按功能封装](#62-按功能封装)
  - [6.3 端口和适配](#63-端口和适配)
  - [6.4 宽松的分层架构](#64-宽松的分层架构)
  - [6.5 按组件封装](#65-按组件封装)
  - [6.6 总结](#66-总结)
