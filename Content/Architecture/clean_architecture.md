- [Clean Architecture 读书笔记](#clean-architecture-读书笔记)
  - [编程范式](#编程范式)
    - [结构化编程（Structured Programming）](#结构化编程structured-programming)
    - [面向对象编程（Object Oriented Programming）](#面向对象编程object-oriented-programming)
    - [函数式编程（Functional Programming）](#函数式编程functional-programming)
    - [总结](#总结)
  - [设计原则 - SOLID 原则](#设计原则---solid-原则)
    - [单一职责（SRP）](#单一职责srp)
    - [开闭原则（OCP）](#开闭原则ocp)
    - [里式替换（LSP）](#里式替换lsp)
  ...
  - [组件](#组件)
    - [什么是组件](#什么是组件)
  - [组件的聚合](#组件的聚合)
    - [复用/发布等同原则（REP原则 - Release Reuse Equivalency Principle）](#复用发布等同原则rep原则---release-reuse-equivalency-principle)
    - [共同闭包原则 （CCP原则 - the Common Closure Principle）](#共同闭包原则-ccp原则---the-common-closure-principle)
      - [CCP与SRP原则的相似点:](#ccp与srp原则的相似点)
    - [共同复用原则（CRP原则 - Composite Reuse Principle）](#共同复用原则crp原则---composite-reuse-principle)
      - [与ISP原则的关系](#与isp原则的关系)
    - [三个规则的Treadeoff](#三个规则的treadeoff)
  - [组件耦合](#组件耦合)
    - [无依赖环原则（ADP - Acyclic Dependency Principle）](#无依赖环原则adp---acyclic-dependency-principle)
    - [自上而下的设计](#自上而下的设计)
    - [稳定依赖原则 （SDP - Stabilization Dependency Principle）](#稳定依赖原则-sdp---stabilization-dependency-principle)
    - [稳定抽象原则（SAP - Stable Abstractions Principle）](#稳定抽象原则sap---stable-abstractions-principle)
      - [高阶策略应该放在哪里？](#高阶策略应该放在哪里)
      - [稳定抽象原则简介](#稳定抽象原则简介)
  - [软件架构](#软件架构)
    - [什么是软件架构？软件架构师工作内容是什么？](#什么是软件架构软件架构师工作内容是什么)
    - [开发（development）](#开发development)
    - [部署（Deployment）](#部署deployment)
    - [维护（Maintenance）](#维护maintenance)
    - [保持可选项](#保持可选项)
  - [独立性](#独立性)
    - [用例](#用例)
    - [运行](#运行)
  ...
  - [实现细节](#实现细节)
    - [数据库只是细节](#数据库只是细节)
      - [假设磁盘不存在会怎么样？](#假设磁盘不存在会怎么样)
      - [实现细节](#实现细节-1)
    - [web只是细节](#web只是细节)
    - [应用程序框架只是细节](#应用程序框架只是细节)
      - [单向婚姻](#单向婚姻)
      - [风险](#风险)
      - [解决方案](#解决方案)
      - [总结](#总结-1)
