...
type Message struct {
        ID                    int64                              
        Type                  LarkImMessageConstants.MessageType 
        CreateTime            int64                              
        FromID                *int64                             
        Content               *Content                           
        Extra                 *Extra                             
...
func PushMessage(ctx context.Context, request *message.PushMessageRequest) (*message.PushMessageResponse, error) {
   log := common.Logger(ctx).WithField("method", "app.PushMessage")
...
type pushDomainServiceImpl struct {
   repo repository.PushRepository // Domain层主要通过依赖倒置方式调用Infra实现
}
...
func NewPushRepo() PushRepository {
   return persistence.PackRepoImpl{} // 这里可以用依赖注入
}
