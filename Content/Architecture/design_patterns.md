- [前言](#前言)
	...
	- [面向对象、设计原则、设计模式、编程规范、重构，这五者有何关系？](#面向对象设计原则设计模式编程规范重构这五者有何关系)
- [面向对象](#面向对象)
	- [编程范式](#编程范式)
	- [什么是面向对象编程？](#什么是面向对象编程)
	...
	- [Constants、Utils 类设计问题](#constantsutils-类设计问题)
	- [抽象类和接口](#抽象类和接口)
		- [抽象类](#抽象类)
		- [接口特性](#接口特性)
		- [抽象类和接口能解决什么编程问题](#抽象类和接口能解决什么编程问题)
		- [抽象类和接口的应用场景区别](#抽象类和接口的应用场景区别)
...
- [设计原则](#设计原则)
	- [SOLID 原则](#solid-原则)
		- [单一职责（Single Responsibility Principle）](#单一职责single-responsibility-principle)
		- [开闭原则（Open Closed Principle）](#开闭原则open-closed-principle)
		- [里式替换（Liskov Substitution Principle）](#里式替换liskov-substitution-principle)
		- [接口隔离原则（Interface Segregation Principle）](#接口隔离原则interface-segregation-principle)
		- [依赖反转原则（Dependency Inversion Principle）](#依赖反转原则dependency-inversion-principle)
	- [KISS原则](#kiss原则)
	- [YAGNI原则](#yagni原则)
	- [DRY 原则（Don’t Repeat Yourself）](#dry-原则dont-repeat-yourself)
	- [代码复用性](#代码复用性)
		- [减少代码耦合](#减少代码耦合)
		- [满足单一职责原则](#满足单一职责原则)
		- [模块化](#模块化)
		- [业务与非业务逻辑分离](#业务与非业务逻辑分离)
		- [通用代码下沉](#通用代码下沉)
		- [继承、多态、抽象、封装](#继承多态抽象封装)
		- [应用模板等设计模式](#应用模板等设计模式)
	- [迪米特法则/最小知识原则](#迪米特法则最小知识原则)
		- [如何理解“高内聚、松耦合”？](#如何理解高内聚松耦合)
		- [如何理解“迪米特法则”？](#如何理解迪米特法则)
	- [为什么要分 MVC 三层开发？](#为什么要分-mvc-三层开发)
		- [分层能起到代码复用的作用](#分层能起到代码复用的作用)
		- [分层能起到隔离变化的作用](#分层能起到隔离变化的作用)
		- [分层能起到隔离关注点的作用](#分层能起到隔离关注点的作用)
		- [分层能提高代码的可测试性](#分层能提高代码的可测试性)
		- [分层能应对系统的复杂性](#分层能应对系统的复杂性)
	- [BO、VO、Entity 存在的意义是什么？](#bovoentity-存在的意义是什么)
		- [相对于每层定义各自的数据对象来说，是不是定义一个公共的数据对象更好些呢？](#相对于每层定义各自的数据对象来说是不是定义一个公共的数据对象更好些呢)
		- [既然 VO、BO、Entity 不能合并，那如何解决代码重复的问题呢？](#既然-voboentity-不能合并那如何解决代码重复的问题呢)
		- [代码重复问题解决了，那不同分层之间的数据对象该如何互相转化呢？](#代码重复问题解决了那不同分层之间的数据对象该如何互相转化呢)
- [规范与重构](#规范与重构)
	- [为什么要重构](#为什么要重构)
	- [重构的对象](#重构的对象)
	- [什么时候重构](#什么时候重构)
	...
	- [关于重构思考](#关于重构思考)
	- [函数出错应该返回啥？](#函数出错应该返回啥)
- [代码质量、面向对象、设计模式、规范与重构总结](#代码质量面向对象设计模式规范与重构总结)
	- [代码质量评判标准](#代码质量评判标准)
	- [面向对象](#面向对象-1)
	- [设计模式](#设计模式)
	- [规范与重构](#规范与重构-1)
- [设计模式与范式：创建型](#设计模式与范式创建型)
	- [单例设计模式（Singleton Design Pattern）](#单例设计模式singleton-design-pattern)
		- [单例的定义](#单例的定义)
		- [单例的用处](#单例的用处)
		- [单例存在哪些问题](#单例存在哪些问题)
		- [单例有什么替代解决方案？](#单例有什么替代解决方案)
	- [工厂模式（Factory Design Pattern）](#工厂模式factory-design-pattern)
		- [简单工场（Simple Factory）](#简单工场simple-factory)
		- [工厂方法模式](#工厂方法模式)
		- [那什么时候该用工厂方法模式，而非简单工厂模式呢？](#那什么时候该用工厂方法模式而非简单工厂模式呢)
		- [抽象工厂（Abstract Factory）](#抽象工厂abstract-factory)
		- [工厂模式和 DI 容器有何区别？](#工厂模式和-di-容器有何区别)
		- [DI 容器的核心功能有哪些？](#di-容器的核心功能有哪些)
	- [构建者模式（Builder pattern）](#构建者模式builder-pattern)
	- [原型模式(Prototype Pattern)](#原型模式prototype-pattern)
- [设计模式与范式：结构型](#设计模式与范式结构型)
	- [代理模式（Proxy Design Pattern）](#代理模式proxy-design-pattern)
		- [动态代理（Dynamic Proxy）](#动态代理dynamic-proxy)
		- [代理模式的应用场景](#代理模式的应用场景)
	- [桥接模式（Bridge Design Pattern）](#桥接模式bridge-design-pattern)
