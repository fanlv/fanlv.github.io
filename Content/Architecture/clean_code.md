...
- [函数](#函数)
	...
	- [每个函数一个抽象层级](#每个函数一个抽象层级)
		- [自顶向下读代码：向下规则](#自顶向下读代码向下规则)
	- [switch](#switch)
	- [函数参数](#函数参数)
	- [标识参数](#标识参数)
	- [动词与关键字](#动词与关键字)
- [注释](#注释)
	- [注释不能美化糟糕的代码](#注释不能美化糟糕的代码)
	- [好的注释场景](#好的注释场景)
	- [坏注释场景](#坏注释场景)
- [格式](#格式)
	- [垂直格式](#垂直格式)
...
- [类](#类)
	...
	- [单一权责原则](#单一权责原则)
	- [内聚](#内聚)
	- [保持内聚性就会得到许多短小的类](#保持内聚性就会得到许多短小的类)
	- [隔离修改](#隔离修改)
- [系统](#系统)
	- [将系统的构造与使用分开](#将系统的构造与使用分开)
	- [工厂](#工厂)
	- [依赖注入](#依赖注入)
- [并发编程](#并发编程)
	- [为什么要并发](#为什么要并发)
...
- [味道与启发](#味道与启发)
	- [注释](#注释-1)
		...
		- [注释掉的代码](#注释掉的代码)
	- [环境](#环境)
		- [需要多步才能实现的构建](#需要多步才能实现的构建)
		- [需要多步才能做到的测试](#需要多步才能做到的测试)
	- [函数](#函数-1)
		- [过多的参数](#过多的参数)
		- [输出参数](#输出参数)
		- [标识参数](#标识参数-1)
		- [死函数](#死函数)
	- [一般性问题](#一般性问题)
		- [一个源文件中存在多种语言](#一个源文件中存在多种语言)
		- [明显的行为未被实现](#明显的行为未被实现)
		- [不正确的边界行为](#不正确的边界行为)
		- [忽视安全](#忽视安全)
		- [重复](#重复)
		- [在错误的抽象层级上的代码](#在错误的抽象层级上的代码)
		- [基类依赖于派生类](#基类依赖于派生类)
		- [信息过多](#信息过多)
		- [死代码](#死代码)
		- [垂直分隔](#垂直分隔)
		...
		- [不恰当的静态方法](#不恰当的静态方法)
		- [使用解释性变量](#使用解释性变量)
		- [函数名称应该表达其行为](#函数名称应该表达其行为)
		- [理解算法](#理解算法)
		- [把逻辑依赖改为物理依赖](#把逻辑依赖改为物理依赖)
		- [用多态替代 If/Else 或 Switch/Case](#用多态替代-ifelse-或-switchcase)
		- [遵循标准约定](#遵循标准约定)
		- [用命名常量替代魔术数](#用命名常量替代魔术数)
		- [准确](#准确)
		- [结构甚于约定](#结构甚于约定)
		- [封装条件](#封装条件)
		- [避免否定性条件](#避免否定性条件)
		...
		- [别随意](#别随意)
		- [封装边界条件](#封装边界条件)
		- [函数应该只在一个抽象层级上](#函数应该只在一个抽象层级上)
		- [在较高层级放置可配置数据](#在较高层级放置可配置数据)
		- [避免传递浏览](#避免传递浏览)
	- [Java](#java)
		- [采用描述性名称](#采用描述性名称)
		- [名称应与抽象层级相](#名称应与抽象层级相)
		- [尽可能使用标准命名法](#尽可能使用标准命名法)
		- [无歧义的名称](#无歧义的名称)
		- [为较大作用范围选用较长名称](#为较大作用范围选用较长名称)
		- [避免编码](#避免编码)
		- [名称应该说明副作用](#名称应该说明副作用)
	- [测试](#测试)
		- [测试不足](#测试不足)
		- [使用覆盖率工具](#使用覆盖率工具)
		- [别略过小测试](#别略过小测试)
		- [被忽略的测试就是对不确定事物的疑问](#被忽略的测试就是对不确定事物的疑问)
		- [测试边界条件](#测试边界条件)
		- [全面测试相近的缺陷](#全面测试相近的缺陷)
		- [测试失败的模式有启发性](#测试失败的模式有启发性)
		- [测试覆盖率的模式有启发性](#测试覆盖率的模式有启发性)
		- [测试应该快速](#测试应该快速)
